...
export class HttpApiService {
  ...
  constructor( 
    private http :HttpClient ) {
      this.urlServer = "https://localhost:44399/" ;// inject(API_CLIENT_SERVICE_BASE_URL);
  ...
  get(url:string):Observable<any>{
      ...
      // this.http.get<T>(this.urlServer + url , {headers: this.getJsonHttpHeaders()}).pipe(
  ...
  put<T>(url: string , data?: any) : Observable<any>{
    return this.http.put<T>(this.urlServer + url, data, { headers: this.getJsonHttpHeaders()})    
     .pipe(
     //  // retry(3), // retry a failed request up to 3 times
       catchError(this.handleError) // then handle the error
     );
  ...
  delete<T>(url: string, id:number): Observable<any>{
    return this.http.delete<any>(this.urlServer+ url+ id, {headers: this.getJsonHttpHeaders()})
    .pipe(
      // retry(3), // retry a failed request up to 3 times
       catchError(this.handleError) // then handle the error
     );
